...
def about(req):
    ...
    if req.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                send_mail(subject, body, settings.EMAIL_HOST_USER, ["yikkifekno@gufum.com"], fail_silently=False)
                form.save()
...
def create_post(req):
    ...
    if req.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(req, f"Post {id}-{title} was creates successfully")
...
def download(req):
    ...
    for now in Post.objects.all().values_list('title', 'description', 'created_at'):
        writer.writerow(response)
    filename= str(datetime.datetime.now()) + ' ' + 'post.csv'
